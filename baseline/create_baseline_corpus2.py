...
def parse_ratings(filename, ratings_file):
    ...
    for line in ratings_file:
        player_id = '{}_{}'.format(filename, base_player_id)
...
def create_subdocs(match_text, player_info, source):
    ...
    Returns:
        A dict pairing sentences about a player with that player's rating for
        the match.
    """
    ...
    # Convert list of sents into a single string
    for player_id, sents in subdocs_by_player.items():
        text = ' '.join(subdocs_by_player[player_id])
        rating = player_info[player_id]['rating']
        subdocs_by_player[player_id] = [text, rating]
    return subdocs_by_player
...
def create_ratings_corpus(ratings_filenames, source):
    ...
    Returns:
        The corpus mapping a player_id to a tuple of text about that player
        during a particular match and that player's rating for that match.
    """
    subdocs = {}
    for filepath in ratings_filenames:
        ratings_file = open(filepath)
        player_info = parse_ratings(filepath, ratings_file)
        ...
        if match_filepath and os.path.exists(match_filepath):
            ...
            subdocs.update(create_subdocs(match_text, player_info, source))
    return subdocs
...
def main():
    ...
    for player_id in guardian_corpus:
        if player_id in corpus:
            goal_text = corpus[player_id][0]
            guardian_text = guardian_corpus[player_id][0]
            corpus[player_id][0] = ' '.join([goal_text, guardian_text])
        else:
            corpus[player_id] = guardian_corpus[player_id]
    
    corpus_file = open('ratings_corpus2.json', mode='w')

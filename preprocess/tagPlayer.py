...
def tagPlayer(doc, players):
    ...
    matchlist = []
    for fn in players:
        m1=re.finditer(fn, content)
        for match1 in m1:
            matchlist.append([match1.span(),match1.group()])
        m2=re.finditer(fn.split()[-1], content)
        for match2 in m2:
    ...
    with open(doc+".xml","w",-1,"UTF-8") as f:
...
if __name__ == "__main__":
    with open("./playerlist.txt","r",-1,"UTF-8") as f:
        players = f.read().split("\n")
        players.remove("")
    tagPlayer("./test.txt", players)
